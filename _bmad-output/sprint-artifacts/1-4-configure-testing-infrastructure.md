...
- [ ] Task 4: Create sample E2E test for app launch (AC: 2)
  - [ ] Create `tests/e2e/launch.spec.ts`
  - [ ] Configure test to launch Tauri app using `_electron` launcher with correct binary path
  - [ ] Binary path: `src-tauri/target/debug/storyteller` (macOS/Linux) or `src-tauri\\target\\debug\\storyteller.exe` (Windows - escape backslashes)
  - [ ] Test that home screen loads and displays welcome message
  - [ ] Test that "Create New Project" button is clickable
  - [ ] Run `pnpm test:e2e` and verify test passes

- [ ] Task 5: Verify complete test suite (AC: 3)
  - [ ] Run `pnpm test` to execute frontend tests (expect <1 second for 5-6 tests)
  - [ ] Run `pnpm test:e2e` to execute E2E tests
  - [ ] Verify both test suites pass with clear output
  - [ ] Verify total execution time is <30 seconds
  - [ ] Test watch mode: Run `pnpm test --watch` and verify hot reload works (press 'a' for all tests, 'f' for failed only, 'q' to quit)
